...
X_train= X_scaler.fit_transform(train_data[feature])
X_test= X_scaler.fit_transform(test_data[feature])
...
df_lr.to_csv("lr_predictions.csv", encoding='utf-8', index=False)
...
df_nb.to_csv("nb_predictions.csv", encoding='utf-8', index=False)
...
start_time = timeit.default_timer()
X_scaler = StandardScaler()
matrix_train=X_scaler.fit_transform(train_data[feature])
matrix_test=X_scaler.fit_transform(test_data[feature])
#build the model
svm_predict = []
rid=[]
for item in test_data['race_id']:
...
df_svm.to_csv("svm_predictions.csv", encoding='utf-8', index=False)
...
df_nb=pd.DataFrame.from_items(df)
df_nb.to_csv("rf_predictions.csv", encoding='utf-8', index=False)
